...
from models import mBART_Summarizer
import os
...
def run(args):
    ...
    tokenizer = MBartTokenizer.from_pretrained(args.model_type, src_lang=args.src_lang,
        ...
        if accelerator.is_local_main_process:
            ...
            if val_r_avg > best_valid_rouge:
                ...
                if args.save_adapter and args.train_adapter:
                    if args.distributed:
                        model.module.model.save_adapter(
                            args.output_dir + args.checkpoint + '_adapter_ckpt/', args.task_name)
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_adapter", action='store_true',
                        help="whether to save Adapter checkpoint")
    parser.add_argument("--task_name", type=str, default='summarization',
                        help="Name for the task adapter")
    parser.add_argument("--adapter_config", type=str, default='houlsby',
                        help="Adapter configuration: houlsby, pfeiffer, parallel, compacter, etc.")
    parser.add_argument("--adapter_non_linearity", type=str, default='swish',
                        help="Non-linearity fn for adapter: swish, gelu, relu, sigmoid, tanh, etc.")
    parser.add_argument("--adapter_reduction_factor", type=int, default=16,
                        help="The ratio between a model's layer hidden dimension and the bottleneck dimension")
